...
domain: erddap
data-source: ncdcOisst21Agg (NOAA CoastWatch ERDDAP)
...
| Tool | Purpose |
|------|---------|
| `get_data` | Extract SST values for a region and time range |
...
3. Call `get_data` with `variable: "sst"`, target region, and date range.
4. Interpret the returned values: typical range for Mexican seas is 18–32°C.
   - Gulf of Mexico summer peak: 30–32°C
   - Pacific winter minimum (Baja): 14–18°C
5. For high spatial resolution (1km), use `source: "mur_1km"`.
   For pre-calculated anomalies, use `source: "mur_anomaly"`.
